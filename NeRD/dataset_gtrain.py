...
class DatasetTrain(Dataset):
    def __init__(self, root_dir, img_options=None):
        ...
        self.patch_size = self.img_options.get('patch_size', 128)
        self.image_pairs = []
        self._collect_pairs()
        ...
        for scene in sorted(scene_dirs):
            ...
            if len(clean_files) == 1:
                ...
                for rf in rainy_files:
                    self.image_pairs.append((os.path.join(scene_path, rf), clean_path, scene))
            else:
                clean_map = {
    ...
    def _augment(self, rainy, clean):
        ps = self.patch_size
        _, height, width = rainy.shape
        if height < ps or width < ps:
            rainy = TF.resize(rainy, (max(ps, height), max(ps, width)))
            clean = TF.resize(clean, (max(ps, height), max(ps, width)))
        ...
        for scene in sorted(os.listdir(self.root_dir)):
            ...
            for f in sorted(os.listdir(scene_path)):
                if '-R-' in f and f.endswith('.png'):
                    self.samples.append((os.path.join(scene_path, f), f'{scene}_{f[:-4]}'))
    def __len__(self):
