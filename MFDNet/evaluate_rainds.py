...
def main():
    parser = argparse.ArgumentParser(description='Evaluate MFDNet on RainDS')
    parser.add_argument('--result_dir', default='./results/MFDNet/RainDS', type=str, help='Directory of generated images')
    parser.add_argument('--gt_dir', default='d:/Deraining/RLP/rlp/data/RainDS', type=str, help='Directory of GT images')
    ...
    for variant, variant_dir in variant_info:
        ...
        avg_ssim = 0.0
        valid_count = 0
        dataset = DatasetTest(args.gt_dir, variant=variant, rain_type='all')
        variant_result_dir = os.path.join(args.result_dir, variant_dir)
        
        for idx in range(len(dataset)):
            img_path, gt_path, full_name, rain_type = dataset.samples[idx]
            
            res_path = os.path.join(variant_result_dir, full_name + '.png')
