...
class LFSFB(nn.Module):
    ...
    def forward(self, x1, x2, target_size=None):
        res = self.act1(self.FS(x1))
        res_out = self.act2(self.FFU(x2 + res))
        if target_size is not None:
            if res_out.size(2) != target_size[0] or res_out.size(3) != target_size[1]:
                res_out = res_out[:, :, :target_size[0], :target_size[1]]
        return res_out
...
class HRM(nn.Module):
    ...
    def forward(self, x):
        ...
        xR_res = self.CAB_r(xR) + self.lfsfb(res_down_R, res_R, target_size=xR.shape[2:])
