...
from data.Dataset_gtavnightrain import Dataset_GTAVNightRain
...
def main():
    ...
    util.setup_logger('base', opt['path']['log'], 'test_gtavnightrain', 
    ...
    test_dataset = Dataset_GTAVNightRain(opt['datasets']['test_gtavnightrain'])
    ...
    dataset_set = test_dataset.dataset_set
    logger.info(f'Testing on: GTAV-NightRain-{dataset_set}')
    
    # CSV file setup
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    csv_filename = f'NDR_GTAV-NightRain-{dataset_set}_results_{timestamp}.csv'
    ...
    for i in tqdm(range(len(test_dataset)), desc=f'Testing GTAV-{dataset_set}'):
        try:
            ...
            # BRUTAL cleanup after EVERY image
            brutal_memory_cleanup()
    ...
    with open(csv_path, 'w', newline='', encoding='utf-8') as csvfile:
        ...
        writer.writerow([f'# NDR-Restore GTAV-NightRain-{dataset_set} Evaluation Results'])
        writer.writerow([])

        writer.writerow(['# Model Information'])
        writer.writerow(['Model', 'NDR-Restore'])
        writer.writerow(['Pretrained Model', args.model_path])
        writer.writerow(['Test Dataset', f'GTAV-NightRain-{dataset_set}'])
        writer.writerow([])
    ...
    # Console output
    logger.info('='*70)
    logger.info(f'GTAV-NightRain-{dataset_set} Test Results:')
    logger.info('='*70)
