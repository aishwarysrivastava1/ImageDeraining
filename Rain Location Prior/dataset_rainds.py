...
class DatasetTest(Dataset):
    ...
    def _collect(self):
        ...
        if self.rain_type == 'all':
            ...
            raise ValueError(f"Unknown rain_type: {self.rain_type}")
            
        for rain_dir in rain_dirs:
            rain_path = os.path.join(test_dir, rain_dir)
            if not os.path.exists(rain_path):
                print(f"Warning: {rain_path} not found, skipping...")
                continue
                
            files = sorted([f for f in os.listdir(rain_path) if f.endswith('.png')])
            for f in files:
        ...
        print(f"Loaded {len(self.samples)} test images from {test_dir}")
        print(f"Images will be resized to: {self.target_size[0]}×{self.target_size[1]}")
        print(f"Rain types: {rain_dirs}")
