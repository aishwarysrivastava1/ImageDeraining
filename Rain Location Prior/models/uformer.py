'''
# Description: Uformer model
# Reference:
# [1] Yuan et al. Uformer: A General U-Shaped Transformer for Image Restoration, CVPR 2021
'''
...
class Uformer(nn.Module):
    def __init__(self, img_size=256, in_chans=3, out_chans=3, dd_in=3, injection_dim=0,
        ...
        self.output_proj = OutputProj(in_channel=2*embed_dim+injection_dim, out_channel=out_chans, kernel_size=3, stride=1)
        
        # Encoder
        self.encoderlayer_0 = BasicUformerLayer(dim=embed_dim+injection_dim,
        ...
        self.dowsample_0 = dowsample(embed_dim+injection_dim, embed_dim*2)
        ...
        self.decoderlayer_3 = BasicUformerLayer(dim=embed_dim*2+injection_dim,
                            output_dim=embed_dim*2,
                            input_resolution=(img_size,
                                                img_size),
                            depth=depths[8],
    ...
    def forward(self, x, mask=None, rlp=None, feat=None):
        # Input Projection
        if rlp is not None:
            y = self.input_proj(x, rlp, feat)
        else:
            y = self.input_proj(x)
        y = self.pos_drop(y)
        #Encoder
        ...
        return x + y if self.dd_in ==3 else x[:,:3, :, :] + y
