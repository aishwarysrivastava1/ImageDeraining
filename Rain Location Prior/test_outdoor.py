...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Image deraining inference on Outdoor-Rain - BRUTAL OPTIMIZATION')
    ...
    if args.batch_size != 1:
        print(f" WARNING: Forcing batch_size=1 for maximum memory optimization")
        args.batch_size = 1
    ...
    print("="*70)
    print("BRUTAL OPTIMIZATION MODE - Outdoor-Rain")
    print("="*70)
    ...
    if args.max_images:
        print(f"Max images: {args.max_images} (quick testing mode)")
    ...
    print(f"\nLoading test dataset (Outdoor-Rain)...")
    ...
    if args.max_images and args.max_images < len(test_dataset):
        print(f"Limiting to {args.max_images} images for quick testing")
        test_dataset = Subset(test_dataset, range(args.max_images))
    ...
    print(f"✓ Loaded {len(test_dataset)} test images (total dataset: {original_len})")
    result_dir = os.path.join(args.result_dir, args.model_name, 'Outdoor-Rain')
    ...
    with torch.no_grad():
        for i, (input_img, filename) in enumerate(tqdm(test_loader, desc='Testing Outdoor-Rain')):
            try:
    ...
    print("="*70)
    print(f"Dataset: Outdoor-Rain")
    print(f"Successfully processed: {processed_count}")
    print(f"Failed: {failed_count}")
    print(f"Total attempted: {len(test_dataset)}")
    print(f"Results saved to: {result_dir}")
    print("="*70)
    print(f"\n✓ Ready for evaluation!")
    print(f"   Next: python evaluate_outdoor.py")