...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='GTAV-NightRain deraining - BRUTAL OPTIMIZATION')
    ...
    print("="*70)
    print(f"BRUTAL OPTIMIZATION - GTAV-NightRain {args.test_set}")
    print("="*70)
    ...
    if torch.cuda.is_available():
        ...
        print(f"✅ Brutal optimizations: Cleanup every image, Resize to 960×540")
    ...
    test_dataset = DatasetTest(args.input_dir, test_set=args.test_set)
    ...
    result_dir = os.path.join(args.result_dir, args.model_name, f'GTAV-NightRain_{args.test_set}')
    ...
    with torch.no_grad():
        for i, (input_img, filename) in enumerate(tqdm(test_loader, desc=f'Testing GTAV {args.test_set}')):
            try:
    ...
    print(f"Results: {result_dir}")
    print("="*70)
    print(f"\n✓ Next: python evaluate_gtav.py --test_set {args.test_set}")